...
- **.opencode** — OpenCode AI editor configuration, including migration plans and Node.js package definitions for editor extensions and integrations.
...
- **opencode** — Empty Stow package directory, serving as a placeholder for future Stow-managed OpenCode configuration (distinct from `.opencode/`, which holds the active config).
- **Other Stow packages** — Additional Stow-managed configs include tmux, kitty, sketchybar, aerospace, lazygit, git, gnupg, direnv, prettier, p10k, harper-ls, yamlfmt, and stow itself.
